...
/* Create teams, default setup and reset timers/snapshots. */
    ...
    function setVariant(variantId as String) as Void {
        ...
        if (isClockState(RUGBY_STATE_NOT_STARTED)) {
            ...
            System.println("RUGBY|RugbyGameModel|setVariant applied variantId=" + _setup["variantId"] + " variantName=" + _setup["variantName"] + " halfLengthSeconds=" + ("" + _setup["halfLengthSeconds"]) + " sinBinLengthSeconds=" + ("" + _setup["sinBinLengthSeconds"]) + " conversionLengthSeconds=" + ("" + _setup["conversionLengthSeconds"]));
        } else {
    ...
    function adjustIdleMainTimer(deltaMinutes as Number) as Void {
        if (isClockState(RUGBY_STATE_NOT_STARTED)) {
            ...
            System.println("RUGBY|RugbyGameModel|adjustIdleMainTimer deltaMinutes=" + (deltaMinutes == null ? "null" : ("" + deltaMinutes)) + " oldHalf=" + (oldHalf == null ? "null" : ("" + oldHalf)) + " newHalf=" + (newHalf == null ? "null" : ("" + newHalf)));
        }
    }
...
("" +         System.println("RUGBY|RugbyGameModel|startMatch requested nowMs=" + ("" + nowMs) + " clockState=" + _clockState + " halfIndex=" + (currentHalf())) + " activeElapsedMs=" + (_setup["activeElapsedMs"] == null ? "null" : ("" + _setup["activeElapsedMs"])));
        if (isClockState(RUGBY_STATE_NOT_STARTED) || isClockState(RUGBY_STATE_HALF_ENDED)) {
            ...
            System.println("RUGBY|RugbyGameModel|startMatch applied clockState=" + _clockState + " halfStartedAtMs=" + (_setup["halfStartedAtMs"] == null ? "null" : ("" + _setup["halfStartedAtMs"])) + " activeElapsedMs=" + (_setup["activeElapsedMs"] == null ? "null" : ("" + _setup["activeElapsedMs"])));
        } else {
...
/* If running, persist active elapsed ms and mark PAUSED. */
    ...
    function pause(nowMs as Number) as Void {
        System.println("RUGBY|RugbyGameModel|pause requested nowMs=" + ("" + nowMs) + " clockState=" + _clockState);
        if (isClockState(RUGBY_STATE_RUNNING)) {
            _setup["activeElapsedMs"] = activeElapsedMs(nowMs);
            _clockState = RUGBY_STATE_PAUSED;
            System.println("RUGBY|RugbyGameModel|pause applied activeElapsedMs=" + (_setup["activeElapsedMs"] == null ? "null" : ("" + _setup["activeElapsedMs"])) + " clockState=" + _clockState + " pauseReminderIntervalMs=" + ("" + RUGBY_PAUSE_REMINDER_INTERVAL_MS));
        } else {
...
/* If paused, set halfStartedAtMs to now and mark RUNNING. */
    ...
    function resume(nowMs as Number) as Void {
        System.println("RUGBY|RugbyGameModel|resume requested nowMs=" + ("" + nowMs) + " clockState=" + _clockState);
        if (isClockState(RUGBY_STATE_PAUSED)) {
            _setup["halfStartedAtMs"] = nowMs;
            _clockState = RUGBY_STATE_RUNNING;
            System.println("RUGBY|RugbyGameModel|resume applied halfStartedAtMs=" + (_setup["halfStartedAtMs"] == null ? "null" : ("" + _setup["halfStartedAtMs"])) + " activeElapsedMs=" + (_setup["activeElapsedMs"] == null ? "null" : ("" + _setup["activeElapsedMs"])) + " clockState=" + _clockState);
        } else {
...
/* Apply conversion points and clear conversion timer. */
    ...
    function startYellowCard(teamId as String, nowMs as Number) as Number {
        ...
        System.println("RUGBY|RugbyGameModel|startYellowCard teamId=" + teamId + " nowMs=" + ("" + nowMs) + " sanctionId=" + ("" + id) + " sinBinSeconds=" + (_setup["sinBinLengthSeconds"] == null ? "null" : ("" + _setup["sinBinLengthSeconds"])));
        return id;
    }
    ...
    function recordRedCard(teamId as String, nowMs as Number) as Number {
        ...
        System.println("RUGBY|RugbyGameModel|recordRedCard teamId=" + teamId + " nowMs=" + ("" + nowMs) + " sanctionId=" + ("" + id));
        return id;
    }
    ...
    function snapshot(nowMs as Number) as Dictionary {
        _snapshotId += 1;
        System.println("RUGBY|RugbyGameModel|snapshot nowMs=" + ("" + nowMs) + " snapshotId=" + ("" + _snapshotId) + " clockState=" + _clockState + " pending=" + (_pendingConfirmAction == null ? "null" : _pendingConfirmAction));
...
/* Create the conversion timer state anchored to current active elapsed ms. */
    ...
    function startConversionTimer(teamId as String, nowMs as Number) as Void {
        ...
        System.println("RUGBY|RugbyGameModel|startConversionTimer teamId=" + teamId + " nowMs=" + ("" + nowMs) + " startedAtActiveMs=" + ("" + _conversionTimer["startedAtActiveMs"]) + " startedAtMs=" + ("" + _conversionTimer["startedAtMs"]) + " durationSeconds=" + ("" + _conversionTimer["durationSeconds"]) + " clockState=" + _clockState);
    }
    ...
    function dueHapticEvents(conversion as Dictionary?, sanctions as Array<Dictionary>, mainCountdownSeconds as Number) as Array<Dictionary> {
        ...
        if (mainCountdownSeconds != null && mainCountdownSeconds <= 120) {
            ...
            if (_halfTwoMinAlertFiredHalfIndex == null || _halfTwoMinAlertFiredHalfIndex != halfIdx) {
                ...
                System.println("RUGBY|RugbyGameModel|haptic_warning_half halfIndex=" + ("" + halfIdx) + " remainingSeconds=" + ("" + mainCountdownSeconds) + " snapshotId=" + ("" + _snapshotId));
            }
        }
        ...
        if (conversion != null && conversion["active"] && !conversion["nearExpiryAlertFired"] && conversion["remainingSeconds"] <= RUGBY_ALERT_THRESHOLD_SECONDS) {
            ...
            System.println("RUGBY|RugbyGameModel|haptic_warning_conversion remainingSeconds=" + (conversion["remainingSeconds"] == null ? "null" : ("" + conversion["remainingSeconds"]) ) + " snapshotId=" + ("" + _snapshotId));
        }
        for (var i = 0; i < sanctions.size(); i += 1) {
            ...
            if (valueEquals(sanction["cardType"], RUGBY_CARD_YELLOW) && valueEquals(sanction["state"], "active") && !sanction["nearExpiryAlertFired"] && sanction["remainingSeconds"] <= RUGBY_ALERT_THRESHOLD_SECONDS) {
                ...
                System.println("RUGBY|RugbyGameModel|haptic_warning_card sanctionId=" + ("" + sanction["id"]) + " teamId=" + sanction["teamId"] + " remainingSeconds=" + ("" + sanction["remainingSeconds"]) + " snapshotId=" + ("" + _snapshotId));
            }
        }
        return events;
    }
...
/* Compute remaining seconds for a timer, clamped to zero. */
    ...
    function applyAutomaticCountdownExpiry(nowMs as Number) as Void {
        ...
        var halfIndex = currentHalf() as Number;
        var elapsedMs = activeElapsedMs(nowMs) as Number;
        System.println("RUGBY|RugbyGameModel|applyAutomaticCountdownExpiry halfIndex=" + ("" + halfIndex) + " halfCount=" + (_setup["halfCount"] == null ? "null" : ("" + _setup["halfCount"])) + " elapsedMs=" + ("" + elapsedMs) + " finalPeriod=" + (isFinalPeriod() ? "true" : "false"));
        if (isFinalPeriod()) {
            endMatch(nowMs);
    ...
    function preserveYellowCardsForPeriodEnd(elapsedMs as Number) as Void {
        for (var i = 0; i < _sanctions.size(); i += 1) {
            ...
            if (valueEquals(sanction["cardType"], RUGBY_CARD_YELLOW) && valueEquals(sanction["state"], "active")) {
                ...
                if (remaining <= 0) {
                    sanction["state"] = "expired";
                    System.println("RUGBY|RugbyGameModel|preserveYellowCardsForPeriodEnd expired id=" + ("" + sanction["id"]));
                } else {
                    sanction["durationSeconds"] = remaining;
                    sanction["startedAtActiveMs"] = 0;
                    sanction["state"] = "pausedForPeriod";
                    System.println("RUGBY|RugbyGameModel|preserveYellowCardsForPeriodEnd carried id=" + ("" + sanction["id"]) + " teamId=" + sanction["teamId"] + " remainingSeconds=" + ("" + remaining));
                }
            }
        }
    }
    ...
    function resumeCarriedYellowCardsForPeriodStart(nowMs as Number) as Void {
        ...
        for (var i = 0; i < _sanctions.size(); i += 1) {
            ...
            if (valueEquals(sanction["cardType"], RUGBY_CARD_YELLOW) && valueEquals(sanction["state"], "pausedForPeriod")) {
                ...
                System.println("RUGBY|RugbyGameModel|resumeCarriedYellowCardsForPeriodStart id=" + ("" + sanction["id"]) + " teamId=" + sanction["teamId"] + " remainingSeconds=" + ("" + sanction["durationSeconds"]));
            }
        }
    }
/* Force-disable conversion and active yellow timers when match ends. */
    ...
    function pauseForCardIfRunning(nowMs as Number, cardType as String) as Boolean {
        if (isClockState(RUGBY_STATE_RUNNING)) {
            System.println("RUGBY|RugbyGameModel|pauseForCardIfRunning cardType=" + cardType + " nowMs=" + ("" + nowMs));
            pause(nowMs);
            return true;
        }
