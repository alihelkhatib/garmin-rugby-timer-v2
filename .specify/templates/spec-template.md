...
Include edge cases for synchronized timer updates, variant rule boundaries, active card and
conversion timers, haptic alert thresholds, activity recording fallback behavior, and device
screen constraints where relevant.
...
Requirements for UI changes MUST identify which screen structure belongs in Connect IQ
resources and justify any dynamic visuals that require manual drawing in Monkey C.
...
Capture assumptions about rugby variant defaults, Garmin SDK support, supported device lines,
haptic availability, and rugby activity recording constants or fallback behavior.
