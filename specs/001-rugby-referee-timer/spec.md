...
- **FR-023**: The app MUST record the session as a rugby activity in the user's activity history when the target device supports a rugby activity label. When activity recording is enabled, GPS traces MAY be included only if the user explicitly enables GPS collection for that recording; GPS collection MUST be opt-in per recording, respect platform permissions, and be documented to the user. If GPS is not enabled or not available, recordings MUST still capture event timestamps and metadata (scores, cards, substitutions) without GPS.
...
- **FR-043**: The app MUST make stoppage/added time referee-controlled: when the referee pauses the match clock for a stoppage, the match countdown MUST stop completely and only resume when the referee explicitly resumes it. Any manual added/stoppage time MUST be applied only by referee action; automatic estimation or implicit automatic addition of stoppage time is out of scope for v1.
