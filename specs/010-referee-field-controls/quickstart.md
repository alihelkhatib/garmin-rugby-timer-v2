...
- Time-up overtime changes the main timer to a red negative count-up and tracks overtime seconds for at least five simulated minutes.
...
1. Launch on a representative large round profile such as fenix 6 or fenix 7.
2. Start a 15s match.
...
6. Run the first half to `0:00`. Verify the app shows time-up/overtime, the main timer turns red and counts upward as negative time, and the app does not enter half-time automatically.
...
Repeat the readability check on a representative small round profile such as fenix 6s or Forerunner 265s and any rectangular profile supported by the manifest.
...
- `monkeyc -f monkey.jungle -d fenix7 -o build\garmin-rugby-timer-fenix7.prg -y build\codex-garmin-rugby-timer-key.der -w`: PASS on 2026-04-21.
- `monkeydo build\garmin-rugby-timer-fenix7-test.prg fenix7 /t`: BLOCKED on 2026-04-21 because the CLI could not connect to the simulator.
- Size check on 2026-04-21: existing packaged `garmin-rugby-timer-v2.iq` is 4,277,357 bytes; compiled `build\garmin-rugby-timer-fenix7.prg` is 195,916 bytes. No dependency-driven artifact growth was introduced.

## Performance Checks

- Confirm timer updates remain one-second cadence in running, half-time, and time-up states, including the red negative main timer during time-up.
