...
Route physical button behavior by match state so pre-match idle controls adjust the main timer, Select/Start begins the match from the visible idle timer, and active-match controls preserve scoring access. Idle Up/Menu increases the main timer by 1 minute up to the selected variant's normal half length; idle Down decreases by 1 minute down to 00:00; score dialogs remain unavailable before kickoff and after match end. Running, paused, and half-ended states remain active match states for score-dialog availability. The idle screen must keep the existing scoreboard-style layout while centering the main timer and removing or replacing raw internal state text such as "notStarted".
...
**Constraints**: Offline watch app, dark color-blind-friendly UI, no network dependency, no new third-party dependency, no broad UI redesign, no raw internal state text in the idle UI, and no manual drawing unless existing resource-backed layout cannot represent the state
...
- [x] Referee-first scope: The change improves pre-match setup, restores idle Select/Start match start, prevents irrelevant score entry before kickoff, and keeps the idle screen referee-facing without adding analysis or data-entry flows.
...
- [x] Declarative UI resources: The existing resource-backed screen structure remains in place; any layout/static text adjustment is limited to centering the main timer and removing or replacing raw idle-state wording.
...
- Idle Select/Start must remain routed to match start from the currently visible timer value.
- Idle presentation must not expose raw lifecycle identifiers; any status text is referee-facing or omitted.
...
- [data-model.md](data-model.md) defines Match State, Main Timer Value, Idle Button Actions, Variant Timer Bounds, and Score Dialog Availability.
- [contracts/ui-interaction-contract.md](contracts/ui-interaction-contract.md) defines the user-facing button interaction and idle presentation contract.
...
- US1: Adjust Main Timer Before Kickoff, including model/variant bounds, responsive idle button tests, and Select/Start match-start validation.
...
- US4: Keep Idle Screen Readable and Match-Ready, including timer centering and raw-state-label removal validation.
